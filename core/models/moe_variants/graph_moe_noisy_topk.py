...
from core.models.moe_variants.moe_common import BaseRouter, GraphConvExpert, SharedBusEncoder, extract_bus_inputs, graph_size_features, sparse_routing, summarize_routing
from core.utils.registry import registry


@registry.register_model("graph_moe_noisy_topk")
class GraphMoENoisyTopK(nn.Module):
    """Noisy top-k gating MoE with tunable routing exploration."""

    def __init__(self, in_dim: int, hidden_dim: int, out_dim: int, n_layers: int, n_experts: int = 4, top_k: int = 2, noisy_gating_std: float = 0.2, x_start: int = 4, x_end: int = 10):
        super().__init__()
    ...
    def forward(self, data: HeteroData) -> Tensor:
        ...
        probs, sparse, topk = sparse_routing(logits, self.top_k, noisy_std=self.noisy_gating_std, training=self.training)
